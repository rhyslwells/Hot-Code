...
def get_font_size(font_size_value):
    try:
        return int(font_size_value) if font_size_value is not None else 12
    except ValueError:
        ...
        for paragraph in doc.paragraphs:
            ...
            for key in param:
                ...
                if match:
                    ...
                    text_key = f"xxx{placeholder_number}_text"
                    font_size_key = f"xxx{placeholder_number}_font_size"
                    color_key = f"xxx{placeholder_number}_color"
                    
                    # Only proceed if the necessary parameters are present
                    if text_key in param:
